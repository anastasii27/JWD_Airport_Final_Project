package by.epam.tr.controller.constant_parameter;

public final class JSPPageName {

    private JSPPageName(){}

    public static final String RESULT_PAGE = "jsp/result.jsp";
    public static final String USER_PAGE = "jsp/simple_user_page.jsp";
    public static final String ERROR_PAGE = "error.jsp";
    public static final String START_PAGE = "index.jsp";
    public static final String ADMIN_PAGE = "jsp/admin_page.jsp";
    public static final String DISPATCHER_PAGE = "jsp/dispatcher_page.jsp";
    public static final String REGISTER_PAGE = "jsp/register.jsp";
}
